...
1.  Настраиваем `FTP` [FTP](../Уроки/Блок6.md#FTP)
...
Файл настроек для запуска `gunicorn`. Храниться обычно под именем`gunicorn.conf.py`. Для запуска сервера с этими настройками используйте команду `gunicorn -c gunicorn.conf.py ИмяГлавногоПриложения.wsgi` [Документация](https://docs.gunicorn.org/en/stable/settings.html#settings). [Официальный шаблон](https://github.com/benoitc/gunicorn/blob/master/examples/example_config.py)
...
pythonpath = ''
# Слушать указанный ip адрес и порт  '<10.130.0.34:8001>'. Но лучге указать UDS сокет 'unix:/run/gunicorn.sock'
...
accesslog = "" # Путь для вывода лог данных
errorlog = "" # Путь для вывода ошибок
...
> Для того чтобы запустить сервер в одну команду создадим в проекте папку `bin` и `bash` скрипт `bin/runserver.sh`. Потом мы моем выполнить команду `. bin/runserver` и наш север запуститься.
>
> ```bash
> #!/bin/bash
> source /home/Пользователь/ВиртОкр/bin/activate
> gunicorn -c "/home/Пользователь/Проект/gunicorn.config.py" ГлавныйПроект.wsgi
> ```
>
> Не забудьте установить файлу `bin/runserver.sh` право на запуск.
>
> ```bash
> sudo chmod +x bin/runserver.sh
> ```
