...
- ![](_attachments/Pasted%20image%2020210929174626.png)

Реализация на `Python`

- ![](_attachments/Pasted%20image%2020210929181831.png)
...
class File:
	...
	def write(self, text: str):
		if self.is_file_exists(self.__file_name):
			with open(self.__file_name, 'w') as _f:
				# _f.write()
...
if __name__ == '__main__':
	my_file = File("/home/denis/PycharmProjects/consgui/test1.py")
	my_file.write("Python Zen")  # Данные записаны

	print(File.is_file_exists("/home/denis/PycharmProjects/consgui/main.py"))  # Treu
...
==Задача:== Создать класс `User`, с приватными атрибутами `user_name` `email` `photo` `id`. Для приватного атрибута `photo` сделать
геттер и сеттер.
...
Первый и самый важный этап разработки высококачественного класса - соЗадача адекватного интерфейса. Это значит, что
...
Все что здесь делает базовый класс, это модул для работы с сокетами, и
...
- ![](_attachments/Pasted%20image%2020210929233852.png)
